...
public class ChangeFavoriteArtistViewBean extends GenericView{
	private static final Logger logger = LogManager.getLogger( ChangeFavoriteArtistViewBean.class);
	private static final String NAME = "changeFavoriteArtistViewBean";
	private SessionBean sessionBean;
	private String spotifyId;
	private boolean success = false;
	protected MessageSource messageSource;
	public ChangeFavoriteArtistViewBean(MessageSource messageSource, SessionBean session, String spotifyId) {
	...
	protected void setMsgs() {
		mapMsgs.put(ShowArtistDetailsParamsEE.JSON_ARTISTS.getV(),ViewUrlConstants.SHOW_ARTIST_DETAILS_GETPATH);
	    			
	}
	public void process(){
		logger.info("begin changeFavoriteArtistViewBean process method");
		if(sessionBean.getUserWeb()!=null){
			...
			ControllerWsFactory.getInstance(sessionBean)
			               .getChangeStateFavoriteController().changeArtistFavoriteState(spotifyId, userUUID);
			ModelAndView model = new ModelAndView();
			model.addObject(NAME, this);
			model.setViewName(ViewNameConstants.FIND_FAVORITES_ARTISTS_VIEWNAME);
		...
		logger.info("end ChangeFavorritesAlbumViewBean process method");
	}
	...
	protected void addSuccessMsg(String jsonArtist){
		...
		mapMsgs.put(ShowArtistDetailsParamsEE.CHANGE_FAVORITE_ARTIST_STATE.getV(),ViewUrlConstants.CHANGE_FAVORITE_ARTIST_STATE_GETPATH);	
	}
//* GETTETS AND SETTERS */
